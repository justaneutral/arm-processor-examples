...
String.prototype.anchorify = function() {
  if (isChinese(this))
    return b64EncodeUnicode(this).toLowerCase().replace(/[^\w]+/g, '-');
...
Renderer.prototype.expandable = function(level, title, text) {
  ...
  var expandId = 'expandable_' + striphtml(title).anchorify();
...
Renderer.prototype.heading = function(text, level, raw) {
  ...
  var anchorId = this.options.headerPrefix + raw.anchorify();
  ...
  return '<h'
    ...
    + '">'
    + text
    + '<a class="anchor-link glyphicon glyphicon-link" aria-hidden="true" href="#'+anchorId+'"></a>'
    + '</h'
    + level
    + '>'
    '\n';
