   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn && pOut && pOutCn) {
            ...
            status_init = DSPLIB_blk_eswap_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);

            if (status_init == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               DSPLIB_asm(" MARK 0");
               kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
               status_init           = DSPLIB_blk_eswap_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               DSPLIB_asm(" MARK 1");
               TI_profile_stop();
            }
...
/* For every call of DSPLIB_blk_eswap_getHandleSize() the execution history is pushed
   as this function is the anchor point for LDRA in .cpp kernel files.
   Therefore calling DSPLIB_blk_eswap_getHandleSize() to push the execution history
   at the end of coverage test cases. */
   int32_t handleSize_LDRA = DSPLIB_blk_eswap_getHandleSize(&kerInitArgs);
