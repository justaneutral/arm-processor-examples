   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn0 && pIn1 && pOut && pOutCn) {
            ...
            if ((currPrm.dataType == DSPLIB_INT8) || (currPrm.dataType == DSPLIB_INT16)) {
               status_nat_vs_opt =
                   TI_compare_mem_2D((void *) pOut, (void *) pOutCn, 0.001, (double) powf(2, -10), bufParamsOut.dim_x,
            ...
            sprintf(desc, "%s | M = %d, N = %d, K = %d dataType %d", testPatternString, currPrm.M, currPrm.N, currPrm.K,
                    currPrm.dataType);
#if (!defined(EVM_TEST))
            ...
            TI_profile_add_test(testNum++, currPrm.M * currPrm.N * currPrm.K, warm_cycles, currPrm.K, currentTestFail,
                                desc);
         }
         else {
...
int coverage_test_main()
{
   ...
   while (testNum <= 1013) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle = DSPLIB_FUNCTION_NATC;
         status_nat            = DSPLIB_matMul_fixed_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                      &bufParamsOutTemp, &kerInitArgs);
         kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt            = DSPLIB_matMul_fixed_init_checkParams(NULL, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                      &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1001:
         ...
         kerInitArgs.funcStyle = DSPLIB_FUNCTION_NATC;
         status_nat            = DSPLIB_matMul_fixed_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                      &bufParamsOutTemp, &kerInitArgs);

         kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt            = DSPLIB_matMul_fixed_init_checkParams(handle, &bufParamsIn0Temp, &bufParamsIn1Temp,
                                                                      &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1004:
         ...
         break;
      case 1005:
         bufParamsIn0Temp.data_type = DSPLIB_FLOAT32;
         bufParamsIn1Temp.data_type = bufParamsIn1.data_type;
         bufParamsOutTemp.data_type = bufParamsOut.data_type;
         ...
         kerInitArgs.funcStyle = DSPLIB_FUNCTION_NATC;
         status_nat =
             DSPLIB_matMul_fixed_init(handle, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp, &kerInitArgs);
      ...
      case 1012:
         ...
         status_nat =
             DSPLIB_matMul_fixed_init(handle, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp, &kerInitArgs);
         status_nat = DSPLIB_matMul_fixed_exec_checkParams(handle, NULL, pIn1, pOut);
         ...
         status_opt =
             DSPLIB_matMul_fixed_init(handle, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp, &kerInitArgs);
         status_opt      = DSPLIB_matMul_fixed_exec_checkParams(handle, pIn0, NULL, pOut);
         currentTestFail = ((status_nat != DSPLIB_ERR_NULL_POINTER) || (status_opt != DSPLIB_ERR_NULL_POINTER));
         break;
      case 1013:
         bufParamsIn0Temp.data_type = bufParamsIn0.data_type;
         ...
         status_nat =
             DSPLIB_matMul_fixed_init(handle, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp, &kerInitArgs);
         status_nat = DSPLIB_matMul_fixed_exec_checkParams(handle, pIn0, pIn1, NULL);
         ...
         status_opt =
             DSPLIB_matMul_fixed_init(handle, &bufParamsIn0Temp, &bufParamsIn1Temp, &bufParamsOutTemp, &kerInitArgs);
         status_opt = DSPLIB_matMul_fixed_exec_checkParams(handle, pIn0, pIn1, NULL);
...
/* For every call of DSPLIB_matMul_fixed_getHandleSize() the execution history is pushed
   as this function is the anchor point for LDRA in .cpp kernel files.
   Therefore calling DSPLIB_matMul_fixed_getHandleSize() to push the execution history
   at the end of coverage test cases. */
   int32_t handleSize_LDRA = DSPLIB_matMul_fixed_getHandleSize(&kerInitArgs);
