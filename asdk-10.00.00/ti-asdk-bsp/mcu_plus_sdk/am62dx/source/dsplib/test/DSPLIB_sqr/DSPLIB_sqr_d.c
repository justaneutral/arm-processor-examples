   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn && pOut && pOutCn) {
            ...
            status_init           = DSPLIB_sqr_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
...
/* For every call of DSPLIB_sqr_getHandleSize() the execution history is pushed
   as this function is the anchor point for LDRA in .cpp kernel files.
   Therefore calling DSPLIB_sqr_getHandleSize() to push the execution history
   at the end of coverage test cases. */
   int32_t handleSize_LDRA = DSPLIB_sqr_getHandleSize(&kerInitArgs);
