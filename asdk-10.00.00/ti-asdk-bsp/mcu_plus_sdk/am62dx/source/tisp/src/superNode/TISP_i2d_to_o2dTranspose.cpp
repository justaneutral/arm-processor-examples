...
i2d_to_o2dTranspose<dTypeIn, dTypeOut>::i2d_to_o2dTranspose(opVec    &_myOpVec,
                                                            dTypeIn  *_pIn,
                                                            dTypeOut *_pOut,
                                                            dTypeIn  *_pInBlock,
                                                            dTypeOut *_pOutBlock,
...

{
   ...
   // set transfer dimenssion structure
   dmaUtils<dTypeIn>::dmaAutoIncSetupXferPropOut2DTranspose(_width, _height, _blockWidth, _blockHeight, _inPitch,
                                                            _outPitch, &transferDimOut);
   // assign channel number
   chIdOut[0] = 1;
...
template <typename dTypeIn, typename dTypeOut> void i2d_to_o2dTranspose<dTypeIn, dTypeOut>::exec()
{
   superNode::exec();
}

template class TISP::SuperNode::i2d_to_o2dTranspose<float, float>;
template class TISP::SuperNode::i2d_to_o2dTranspose<double, double>;
