 ...
 * This example can achieve 1 block of latency in the audio path if the loaded layout matches the AWEInstance's fundamental blocksize
 * If the blocksizes are not equal, there will be 2 blocks of latency. 
 * This is due to the fact that this example uses signalling for the pumps, and awe_audioPump is NOT
 * called inline with the audio callback. 
 * Examples */
