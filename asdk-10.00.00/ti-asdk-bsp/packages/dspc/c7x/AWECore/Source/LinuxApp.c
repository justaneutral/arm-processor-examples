...
int main( int argc, const char* argv[] )
{
	...
	if (!quiet)
	{   
        AWEBuildVersionInfo_t aweVersion;
        ret =  awe_getBuildVersionInfo(&aweVersion);

        if (aweVersion.hotFixVer)
        {
            // If hot fix version is not empty
            printf("\nAWECore version %u.%c.%u.%u.%c (build %u) initialized with the following parameters\n\n", 
                        aweVersion.tuningVer, aweVersion.majorVer, aweVersion.minorVer, aweVersion.procVer, 
                        aweVersion.hotFixVer, aweVersion.buildNumber);
        }
        else
        {
            printf("\nAWECore version %u.%c.%u.%u (build %u) initialized with the following parameters\n\n", 
                        aweVersion.tuningVer, aweVersion.majorVer, aweVersion.minorVer, aweVersion.procVer, 
                        aweVersion.buildNumber);
        }
