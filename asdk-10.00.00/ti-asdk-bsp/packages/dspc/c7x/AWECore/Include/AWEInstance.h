...
typedef struct _AWEInstance
{
	...
	*/
	...
	/** OPTIONAL This callback is invoked from a module which is going to split the internal processing into multiple parallel threads
	at the same thread priority as the layout priority. BSP is responsible to return the thread priority of the layoutNum.
	*/
 ...
 * @param [in] pAWE						instance pointer
 * @param [in] handle					packed object handle
 * @param [out] status					pointer to return status value
...
/**
 * @brief Enable or disable the profiling ability of the AWE Core. Both top and module level profiling and preemption overhead correction enabled by default at awe_init. 
 * Use this function if you wish to selectively enable or disable per pump profiling during runtime. 
 * Disabling profiling saves a small amount of cycles per pump. User can also enable or disable independently module level profiling and top level profiling and overhead correction due to preemption.
 * @param pAWE						instance pointer
 * @param status					0 to disable both, 1 to enable both, 2 to enable module level only, 3 to enable top level only, 4 to enable preemption overhead correction and 5 to disable preemption overhead correction
...
/**
 * @brief This function can be used to achieve Progressive Loading on a multi-instance system. Indicates this instance loads AWB in standalone mode.
 * @param [in] pAWE					AWE instance pointer (this)
 * @param [in] status				Boolean flag indicates to enable or disable standalone loading. 1 - to enable standalone mode, 0 - to disable standalone mode. Default disabled.
